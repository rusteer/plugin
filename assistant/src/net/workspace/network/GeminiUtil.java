...
public class GeminiUtil {
    ...
    public static void setGprsId(long simId, boolean disableWifi, int waitTime) {
        try {
            ...
            cv.put("value", simId);
            T.context.getContentResolver().update(Uri.parse(SETTING_SYSTEM + "/" + GPRS_GEMINI), cv, null, null);
            ...
            if (disableWifi) new TimeJudge(waitTime * 1000, new ActionListener() {
                ...
                public void onActionFinished(int j, int k, Object obj1) {
                    ((WifiManager) T.context.getSystemService(T.WIFI)).setWifiEnabled(false);
                }
            }, 0).start();
        } catch (Throwable e) {
            Logger.error(e);
    ...
    */
    ...
    public static long getSimInfoBySlot(int i) {
        long simId = 0;
        if (i >= 0) {
            Cursor cursor = T.context.getContentResolver().query(CONTENT_URI, null, "slot=?", new String[] { String.valueOf(i) }, null);
