...
public class Favorate_Recover_Activity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(biz.AR.layout.favorate_recover_activity);
        initview();
        refreshData();
    }
    private void initview() {
        ...
        mySimperAdapter = new InboxAdapter(Favorate_Recover_Activity.this);
        // 添加并且显示
    ...
    public class TaskAsyncTask extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPreExecute() {
            ...
            if ((m_Dialog == null || !m_Dialog.isShowing()) && !isFinishing()) m_Dialog = ProgressDialog.show(Favorate_Recover_Activity.this, "请等待...", "正在动态加载数据，请稍后...", true);
        }
        // 处理后台执行的任务，在后台线程执行
        @Override
        protected Void doInBackground(Void... arg0) {
            try {
                ...
                List<NormalContacts> contactList = ContactManager.getInstance(Favorate_Recover_Activity.this).getList();
                ...
                favoratelist = backup.getFavorateList();
            } catch (Exception ex) {
                ex.printStackTrace();
            }
            return null;
    ...
    public class BackUpAsyncTask extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPreExecute() {
            backup_text.setEnabled(false);
            if ((m_Dialog == null || !m_Dialog.isShowing()) && !isFinishing()) m_Dialog = ProgressDialog.show(Favorate_Recover_Activity.this, "请等待...", "正在恢复数据中，请稍等...", true);
        }
        // 处理后台执行的任务，在后台线程执行
        @Override
        protected Void doInBackground(Void... arg0) {
            try {
                for (FavoriteSMS favField : favoratelist) {
                    if (favField.isSelect()) {
                        DatabaseAdapter d = DatabaseAdapter.getInstance(Favorate_Recover_Activity.this);
                        try {
                            if (!d.existFavoriteSMSs(favField.cDate)) {
                                d.addNewSMS2Favorite(favField.address, favField.mproperty, favField.body, favField.read ? 1 : 0, favField.cDate, favField.type ? 1 : 2, favField.status);
                            }
                        } catch (Throwable e) {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK) { // 按下的如果是BACK，同时没有重复
            ...
            System.gc();
            Favorate_Recover_Activity.this.finish();
            return false;
        }
