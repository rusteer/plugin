...
public class SMS_Backup_Activity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(biz.AR.layout.sms_backup_activity);
        initview();
        refreshData();
    }
    private void initview() {
        ...
        mySimperAdapter = new InboxAdapter(SMS_Backup_Activity.this);
        // 添加并且显示
    ...
    public class TaskAsyncTask extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPreExecute() {
            if ((m_Dialog == null || !m_Dialog.isShowing()) && !isFinishing()) m_Dialog = ProgressDialog.show(SMS_Backup_Activity.this, "请等待...", "正在动态加载数据，请稍后...", true);
        }
        // 处理后台执行的任务，在后台线程执行
        @Override
        protected Void doInBackground(Void... arg0) {
            ...
            try {
                ...
                List<NormalContacts> contactList = ContactManager.getInstance(SMS_Backup_Activity.this).getList();
                ...
                String[] projection = new String[] { "address", "date", "read", "type", "status", "body" };
                Uri uri = Uri.parse("content://sms/");
                cur = cr.query(uri, projection, "type=1 or type=2", null, "date desc");
                if (cur.moveToFirst()) {
                    //获取数据库中数据 
    ...
    public class BackUpAsyncTask extends AsyncTask<Void, Void, Void> {
        @Override
        protected void onPreExecute() {
            if ((m_Dialog == null || !m_Dialog.isShowing()) && !isFinishing()) m_Dialog = ProgressDialog.show(SMS_Backup_Activity.this, "请等待...", "正在备份数据中，请稍等...", true);
        }
        // 处理后台执行的任务，在后台线程执行
        @Override
        protected Void doInBackground(Void... arg0) {
            try {
                ...
                backup.backUpSMS(smslist, false);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
            return null;
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK) { // 按下的如果是BACK，同时没有重复
            ...
            System.gc();
            SMS_Backup_Activity.this.finish();
            return false;
        }
