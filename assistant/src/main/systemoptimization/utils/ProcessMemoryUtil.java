...
public class ProcessMemoryUtil {
        ...
        for (String row : rows) {
            ...
            if (tempString.indexOf("PID") == -1) {
                if (bIsProcInfo == true) {
                   ...
                   // System.out.println("运行信息：" + tempString);
    ...
    public String getMemInfoByPid(int pid) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            tempPid = Integer.parseInt(tempPidString.trim());
            if (tempPid == pid) {
                result = "CPU:" + item[INDEX_CPU] + "  内存:" + item[INDEX_RSS];
                break;
            }
    ...
    public String getCPUSizeByPid(int pid) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            tempPid = Integer.parseInt(tempPidString.trim());
            if (tempPid == pid) {
                int size = item[INDEX_CPU].length();
                result = item[INDEX_CPU].substring(0, size - 1);
                break;
            }
        }
        return result;
