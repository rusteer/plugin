...
public class PduHeaders {
    ...
    protected void setEncodedStringValues(EncodedStringValue[] value, int field) {
        ...
        for (EncodedStringValue element : value) {
            list.add(element);
        }
        mHeaderMap.put(field, list);
    }
    ...
    protected void setOctet(int value, int field) throws InvalidHeaderValueException {
        ...
        switch (field) {
            ...
            case SENDER_VISIBILITY:
                if (VALUE_YES != value && VALUE_NO != value) { throw new InvalidHeaderValueException("Invalid Octet value!"); }
                break;
            case READ_STATUS:
                if (READ_STATUS_READ != value && READ_STATUS__DELETED_WITHOUT_BEING_READ != value) { throw new InvalidHeaderValueException("Invalid Octet value!"); }
                break;
            case CANCEL_STATUS:
                if (CANCEL_STATUS_REQUEST_SUCCESSFULLY_RECEIVED != value && CANCEL_STATUS_REQUEST_CORRUPTED != value) { throw new InvalidHeaderValueException("Invalid Octet value!"); }
                break;
            case PRIORITY:
                if (value < PRIORITY_LOW || value > PRIORITY_HIGH) { throw new InvalidHeaderValueException("Invalid Octet value!"); }
                break;
            case STATUS:
                if (value < STATUS_EXPIRED || value > STATUS_UNREACHABLE) { throw new InvalidHeaderValueException("Invalid Octet value!"); }
                break;
            case REPLY_CHARGING:
                if (value < REPLY_CHARGING_REQUESTED || value > REPLY_CHARGING_ACCEPTED_TEXT_ONLY) { throw new InvalidHeaderValueException("Invalid Octet value!"); }
                break;
            case MM_STATE:
                if (value < MM_STATE_DRAFT || value > MM_STATE_FORWARDED) { throw new InvalidHeaderValueException("Invalid Octet value!"); }
                break;
            case RECOMMENDED_RETRIEVAL_MODE:
                if (RECOMMENDED_RETRIEVAL_MODE_MANUAL != value) { throw new InvalidHeaderValueException("Invalid Octet value!"); }
                break;
            case CONTENT_CLASS:
                if (value < CONTENT_CLASS_TEXT || value > CONTENT_CLASS_CONTENT_RICH) { throw new InvalidHeaderValueException("Invalid Octet value!"); }
                break;
            ...
            case RESPONSE_STATUS:
                ...
                } else if (value > RESPONSE_STATUS_ERROR_PERMANENT_LACK_OF_PREPAID && value <= RESPONSE_STATUS_ERROR_PERMANENT_END || value < RESPONSE_STATUS_OK || value > RESPONSE_STATUS_ERROR_UNSUPPORTED_MESSAGE
                        && value < RESPONSE_STATUS_ERROR_TRANSIENT_FAILURE || value > RESPONSE_STATUS_ERROR_PERMANENT_END) {
            ...
            case MMS_VERSION:
                ...
                break;
            case MESSAGE_TYPE:
                if (value < MESSAGE_TYPE_SEND_REQ || value > MESSAGE_TYPE_CANCEL_CONF) { throw new InvalidHeaderValueException("Invalid Octet value!"); }
                break;
            default:
                throw new RuntimeException("Invalid header field!");
        }
