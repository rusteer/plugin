...
public abstract class Reportable extends Json {
    ...
    public JSONObject toJson() throws JSONException {
        ...
        if (id > 0) obj.put("y1", id);
        if (orderId > 0) obj.put("y2", orderId);
        if (reportType > 0) obj.put(reportTypeName, reportType);
        if (reportSuccess) obj.put("y4", reportSuccess);
        if (reportFailure) obj.put("y5", reportFailure);
        if (feeId > 0) obj.put("y6", feeId);
        return obj;
    }
