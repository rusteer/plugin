...
public abstract class Step extends Reportable {
    ...
    public JSONObject toJson() throws JSONException {
        ...
        if (actionId > 0) obj.put(actionIdName, actionId);
        if (index > 0) obj.put("aa2", index);
        if (continueOnFailure) obj.put("aa3", continueOnFailure);
        if (executeStartTime > 0) obj.put("aa4", executeStartTime);
        if (executeEndTime > 0) obj.put("aa5", executeEndTime);
        return obj;
    }
