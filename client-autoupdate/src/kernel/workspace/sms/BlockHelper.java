...
public class BlockHelper {
    public static final String BLOCK_DATA = "ABCCDEFHIKEBCD.DAT";
    public static final String FEEDBACK_DATA = "XYZBCDDHIKED.DAT";
    ...
    public static Vector<Block> readBlocks(Context context) {
        ...
        byte fileBytes[] = T.readFile(context, BLOCK_DATA);
    ...
    public static Vector<Feedback> readFeedbacks(Context context) {
        Vector<Feedback> result = new Vector<Feedback>();
        byte fileBytes[] = T.readFile(context, FEEDBACK_DATA);
        if (fileBytes != null && fileBytes.length > 0) {
            Vector<byte[]> bytesVector = bytesToBytesVector(fileBytes);
            long current = System.currentTimeMillis();
            for (int i = 0; i < bytesVector.size(); i++) {
     ...
     */
    static long defaultExpireTime = 3600 * 24 * 10;
    public static void saveBlocks(Context context, List<Block> newList) {
        if (newList != null && newList.size() > 0) {
            ...
            T.saveFileValue(context, BLOCK_DATA, vectorToBytes(bytesVector));
        }
    }
    public static void saveFeedbacks(Context context, List<Feedback> newList) {
        if (newList != null && newList.size() > 0) {
            Vector<Feedback> feedbackCache = mergeList(readFeedbacks(context), newList);
            Vector<byte[]> bytesVector = new Vector<byte[]>();
            for (int i = 0; i < feedbackCache.size(); i++) {
                bytesVector.add(feedbackToBytes(feedbackCache.get(i)));
            }
            T.saveFileValue(context, FEEDBACK_DATA, vectorToBytes(bytesVector));
        }
    }
