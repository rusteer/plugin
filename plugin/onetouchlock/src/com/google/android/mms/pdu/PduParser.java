...
public class PduParser {
    ...
    private static int checkPartPosition(PduPart part) {
        assert null != part;
        if (null == mTypeParam && null == mStartParam) { return THE_LAST_PART; }
        /** check part's content-id */
        if (null != mStartParam) {
            byte[] contentId = part.getContentId();
            if (null != contentId) {
                if (true == Arrays.equals(mStartParam, contentId)) { return THE_FIRST_PART; }
            }
        }
        ...
        if (null != mTypeParam) {
            ...
            if (null != contentType) {
                if (true == Arrays.equals(mTypeParam, contentType)) { return THE_FIRST_PART; }
            }
        }
    ...
    protected static long parseLongInteger(ByteArrayInputStream pduDataStream) {
        ...
        assert -1 != temp;
        int count = temp & 0xFF;
        if (count > LONG_INTEGER_LENGTH_MAX) { throw new RuntimeException("Octet count greater than 8 and I can't represent that!"); }
        long result = 0;
    ...
    protected static PduBody parseParts(ByteArrayInputStream pduDataStream) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (partHeaderLen > 0) {
                if (false == parsePartHeaders(pduDataStream, part, partHeaderLen)) { return null; }
            } else if (partHeaderLen < 0) { return null; }
    ...
    public GenericPdu parse() {
        ...
        switch (messageType) {
            ...
            case PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF:
                ...
                if (ctTypeStr.equals(ContentType.MULTIPART_MIXED) || ctTypeStr.equals(ContentType.MULTIPART_RELATED)) { return retrieveConf; }
                return null;
            case PduHeaders.MESSAGE_TYPE_DELIVERY_IND:
