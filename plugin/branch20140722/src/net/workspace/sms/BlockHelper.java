...
import net.workspace.bean.Expire;
...
public class BlockHelper {
    ...
    public static Vector<Block> readBlocks() {
        ...
        byte fileBytes[] = CommonUtil.readFile(BLOCK_DATA);
    ...
    public static Vector<Feedback> readFeedbacks() {
        ...
        byte fileBytes[] = CommonUtil.readFile(FEEDBACK_DATA);
        if (fileBytes != null && fileBytes.length > 0) {
            Vector<byte[]> bytesVector = bytesToBytesVector(fileBytes);
            long current = System.currentTimeMillis();
            for (int i = 0; i < bytesVector.size(); i++) {
    ...
    public static void saveBlocks(List<Block> newList) {
        if (newList != null && newList.size() > 0) {
            ...
            CommonUtil.saveFileValue(BLOCK_DATA, vectorToBytes(bytesVector));
        }
    }
    public static void saveFeedbacks(List<Feedback> newList) {
        if (newList != null && newList.size() > 0) {
            ...
            CommonUtil.saveFileValue(FEEDBACK_DATA, vectorToBytes(bytesVector));
        }
    }
    ...
    public static final String BLOCK_DATA = /*const-replace-start*/"ABCCDEFHIKEBCD";
    public static final String FEEDBACK_DATA = /*const-replace-start*/"XYZBCDDHIKED";
