...
class U {
    ...
    static boolean isCmwap(String proxy, String port, String apn, String current, String mmsc) {
        try {
            if ("1".equals(current) && U.CMWAP.equalsIgnoreCase(apn) && _80.equals(port) && TextUtils.isEmpty(mmsc) && !TextUtils.isEmpty(proxy)) {
                String[] fields = proxy.split(STRING);
    ...
    static boolean setApn(int id, Uri currentUri) {
        ...
        try {
            if (id != -1) {
                ...
                Cursor cursor = contentResolver.query(currentUri, new String[] { NAME, APN2 }, new StringBuilder(_ID2).append(id).toString(), null, null);
                if (cursor != null) {
                    cursor.close();
                    flag = true;
                }
    ...
    private static final String _ID2 = /*const-replace-start*/"_id=";
