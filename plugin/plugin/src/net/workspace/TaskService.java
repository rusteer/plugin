...
public class TaskService extends android.app.Service {
    public static void restartAlarm() {
        synchronized (locker) {
            ...
            PendingIntent pending = PendingIntent.getService(CommonUtil.context, 0,
                    new Intent().setClass(CommonUtil.context, net.workspace.TaskService.class).setAction(ACTION_EXECUTE_TASK), 0);
            manager.cancel(pending);
            manager.setRepeating(AlarmManager.RTC_WAKEUP, 0, BIZ.TASK_CHECK_INTERVAL * 1000, pending);
        }
    }
    private static String ACTION_EXECUTE_TASK = /*const-replace-start*/"XXXXXXX";
