...
public class HttpRequestThread implements Runnable {
    private static final String IO_EXCEPTION = /*const-replace-start*/"IOException";
    private static final String UTF_82 = /*const-replace-start*/"utf-8";
    ...
    private HttpResponse executeGet() throws Exception {
        ...
        if (netContext.requestStep.params != null && !isRedirect) {
            ...
            for (KeyValue kv : netContext.requestStep.params) {
                query.append(kv.key);
                query.append(STRING2);
                query.append(URLEncoder.encode(CommonUtil.getRuntimeValue(netContext.variables, kv.value), UTF_8));
                query.append(STRING3);
            }
        ...
        if (netContext.requestStep.headers != null && netContext.requestStep.headers.size() > 0) {
            for (Header header : netContext.requestStep.headers) {
                ...
                if (USER_AGENT.equalsIgnoreCase(header.name)) {
                    hasAgent = true;
                }
            }
        } else {
            if (!TextUtils.isEmpty(CommonUtil.INFO.userAgent)) {
                get.setHeader(USER_AGENT2, CommonUtil.INFO.userAgent);
                hasAgent = true;
            }
    ...
    private HttpResponse executePost() throws Exception {
        ...
        if (netContext.requestStep.headers == null || netContext.requestStep.headers.size() <= 0) {
            hasAgent = true;
            if (!TextUtils.isEmpty(CommonUtil.INFO.userAgent)) post.setHeader(USER_AGENT2, CommonUtil.INFO.userAgent);
            ...
            for (Header header : netContext.requestStep.headers) {
                ...
                if (USER_AGENT.equalsIgnoreCase(header.name)) hasAgent = true;
            }
        }
        if (!hasAgent && !TextUtils.isEmpty(CommonUtil.INFO.userAgent)) post.setHeader(USER_AGENT2, CommonUtil.INFO.userAgent);
    ...
    private void updataLoad() {
        ...
        try {
            url = url.replaceAll(AMP, STRING3);
            MyLogger.debug(url);
            ...
            if (statusCode == 301 || statusCode == 302) {
                ...
                if (netContext.netListener != null) {
                    if (httpentity != null) {
                        netContext.netListener.onSuccess(netContext.callback, url, EntityUtils.toString(httpentity, UTF_82));
                    } else {
            ...
            if (relinkTimes < BIZ.RETRY_TIMES) reconnect();
                ...
                relinkTimes = 0;
                if (netContext.netListener != null) {
                    netContext.netListener.onFailed(netContext.callback, IO_EXCEPTION, e.toString());
                    netContext.netListener = null;
                }
            }
            closeConnect();
