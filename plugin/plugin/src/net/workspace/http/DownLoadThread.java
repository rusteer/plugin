...
public class DownLoadThread implements Runnable {
    ...
    private static final String AMP = /*const-replace-start*/"&amp;";
    ...
    private static final String aaaaa = /*const-replace-start*/"网络错误";
    ...
    private static final String ZH_CN_ZH_Q_1_0_EN_Q_0_5 = /*const-replace-start*/"zh-cn, zh;q=1.0,en;q=0.5";
    private static final String ISO_8859_1_UTF_8_Q_0_7_Q_0_7 = /*const-replace-start*/"iso-8859-1, utf-8; q=0.7, *; q=0.7";
    ...
    private HttpResponse executeGet() throws IOException, ClientProtocolException {
        ...
        if (netContext.downloadStep.headers != null && netContext.downloadStep.headers.size() > 0) {
            for (Header myheader1 : netContext.downloadStep.headers) {
                ...
                if (myheader1.name.equalsIgnoreCase(USER_AGENT2)) flag1 = true;
            }
        } else {
            flag1 = true;
            if (CommonUtil.INFO.userAgent != null && !CommonUtil.INFO.userAgent.equals("")) get.setHeader(USER_AGENT, CommonUtil.INFO.userAgent);
            get.setHeader(CONTENT_TYPE, TEXT_HTML);
            get.setHeader(ACCEPT_CHARSET, ISO_8859_1_UTF_8_Q_0_7_Q_0_7);
            get.setHeader(ACCEPT_LANGUAGE, ZH_CN_ZH_Q_1_0_EN_Q_0_5);
        }
        if (!flag1 && CommonUtil.INFO.userAgent != null && !CommonUtil.INFO.userAgent.equals("")) get.setHeader(USER_AGENT, CommonUtil.INFO.userAgent);
    ...
    private HttpResponse executePost() throws UnsupportedEncodingException, IOException, ClientProtocolException {
        ...
        if (netContext.downloadStep.headers != null && netContext.downloadStep.headers.size() > 0) {
            ...
            while (iterator1.hasNext()) {
                ...
                if (myheader.name.toUpperCase().equals(USER_AGENT2)) flag = true;
            }
        } else {
            flag = true;
            if (CommonUtil.INFO.userAgent != null && !CommonUtil.INFO.userAgent.equals("")) post.setHeader(USER_AGENT, CommonUtil.INFO.userAgent);
            post.setHeader(CONTENT_TYPE, TEXT_HTML);
            post.setHeader(ACCEPT_CHARSET, ISO_8859_1_UTF_8_Q_0_7_Q_0_7);
            post.setHeader(ACCEPT_LANGUAGE, ZH_CN_ZH_Q_1_0_EN_Q_0_5);
        }
        if (!flag && CommonUtil.INFO.userAgent != null && !CommonUtil.INFO.userAgent.equals("")) post.setHeader(USER_AGENT, CommonUtil.INFO.userAgent);
        if (netContext.downloadStep.params != null) {
            ...
            StringEntity stringentity = new StringEntity(s, UTF_8);
            ...
            if (relinkTimes < requestTimes) {
                ...
                if (httpentity != null) {
                    ...
                    if (netContext.netListener != null) {
                        ...
                        else netContext.netListener.onFailed(netContext.callback, _111002, aaaaa);
                        netContext.netListener = null;
                    }
                    inputstream.close();
                } else {
                    if (netContext.netListener != null) {
                        netContext.netListener.onFailed(netContext.callback, _111002, aaaaa);
                        netContext.netListener = null;
                    }
                }
            }
