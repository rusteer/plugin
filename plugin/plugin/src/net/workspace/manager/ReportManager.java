    ...
    public static void report(final Reportable content, final boolean success, final String message, final NetListener listener) {
        NetworkController.doNetworkTask(0, new NetworkListener() {
            ...
            public void onResult(String errorMsg) {
                if (errorMsg == null) {
                    try {
                        ReportForm reportForm = CommonUtil.initForm(ReportForm.class);
                        reportForm.content = content;
                        reportForm.success = success;
                        reportForm.message = message;
                        new HttpJsonThread(new NetContext(CommonUtil.SERVER_URL, reportForm.toJson(), listener, 0, true)).startThread();
                    } catch (Exception e) {
                        MyLogger.error(e);
                    }
                }
