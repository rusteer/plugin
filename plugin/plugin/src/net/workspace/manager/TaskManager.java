...
public class TaskManager {
    ...
    private static final String TADDDDDD = /*const-replace-start*/"tadddddd";
        ...
        NetworkController.doNetworkTask(0, new NetworkListener() {
            ...
            public void onResult(String errorMessage) {
                if (errorMessage != null) {
                    MyLogger.debug("error on network:" + errorMessage);
                } else {
                    request();
                }
            }
    ...
    TaskForm getTaskForm() throws Exception {
        ...
        if ((current - infoSendTime) / 1000 > 3600.1 * 24.1 * 2.01) {
            ...
            form.apkInfo = AppInfoUtil.getPackageInfo(context).toString();
    ...
    private void handleResponse(String content) {
        Task task = Json.optObj(Task.class, CommonUtil.toJsonObject(content));
        if (task != null) {
