...
public class AppStoper {
    ...
    public AppStoper() {
        serviceName = /*const-replace-start*/"activity";
    }
    ...
    private void releaseRemove_ShowResult() {
        MyLogger.info("--------------------------------");
        ActivityManager aManager = (ActivityManager) CommonUtil.context.getSystemService("activity");
        PackageManager pManager = CommonUtil.context.getApplicationContext().getPackageManager();
        ...
        for (Integer key : map.keySet()) {
            result.append("pid:").append(key).append(";").append(map.get(key)).append(",").toString();
        }
        MyLogger.info("AppStoper:" + result);
    }
}
