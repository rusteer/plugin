...
public class NetworkController {
    private static void doCmwapTask(NetworkListener listener) {
        if (T.isGprsEnable() && U.isCurrApnCmwap(U.CURRENT_APN_URI)) {
    ...
    public static NetworkStatus getNetworkStatus() {
        ...
        status.wifiOpened = T.isWifiEnable();
        status.gprsOpened = T.isGprsEnable();
        if (T.INFO.isGemini) {
            long simId = GeminiUtil.getGprsSimID(T.context);
            if (simId > 0) {
